...
test("Rover about to drop off should not move in that direction if there have been an sucide earlier", () => {
  ...
  const roverDownN = new Rover("5 5", "0 5 N", "MM");
  expect(roverDownN.getFinalPosition()).toEqual("0 5 N RIP");
  const roverTowardsN = new Rover("5 5", "0 5 N", "MMRM");
  expect(roverTowardsN.getFinalPosition()).toEqual("1 5 E");
});
